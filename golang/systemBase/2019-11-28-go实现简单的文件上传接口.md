...
``` 
- handler 业务逻辑处理
- static 存储静态的 html 文件
- upload 存储上传的文件
## 编写文件上传静态 index.html 页面
在 static 文件夹下新建目录 view。在目录view下新建文件index.html,写入文件内容：
...
```
## 编写项目启动、路由
  在 FILESTOR-SERVER 目录下建立 main.go 启动文件 （也就是main包），写入文件内容：
...
1、标准库：
- [fmt](https://studygolang.com/pkgdoc)：实现了类似C语言printf和scanf的格式化I/O。格式化动作（'verb'）源自C语言但更简单
- [net/http](https://studygolang.com/pkgdoc)：提供了HTTP客户端和服务端的实现
- [io](https://studygolang.com/pkgdoc):Package io provides basic interfaces to I/O primitives.
- [io/ioutil](https://studygolang.com/pkgdoc):Package ioutil implements some I/O utility functions.
